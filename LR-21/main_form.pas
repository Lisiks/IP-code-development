...
procedure Tf_main.bAddClick(Sender: TObject);
begin
       fEdit.eName.Text:= '';
...
procedure Tf_main.bDelClick(Sender: TObject);
begin
  //если данных нет - выходим:
if SG.RowCount = 1 then exit;
//иначе выводим запрос на подтверждение:
if MessageDlg('Требуется подтверждение','Вы действительно хотите удалить контакт "' + SG.Cells[0, SG.Row] + '"?',
...
procedure Tf_main.bSortClick(Sender: TObject);
begin
...
procedure Tf_main.bEditClick(Sender: TObject);
...
var
   MyCont: Contact; //для очередной записи
   f: file of Contact; //файлданных
...
adress:= ExtractFilePath(ParamStr(0));
  //настроимсетку:
SG.Cells[0, 0]:= 'Имя';
...
if not FileExists(adress + 'telephones.dat') then exit;
//иначе файл есть, открываем его для чтения и
//считываемданныевсетку:
try
   AssignFile(f, adress + 'telephones.dat');
   Reset(f);
    //теперь цикл - от первой до последней записи сетки:
...
//считываем новую запись:
      ...
      SG.Cells[1, SG.RowCount-1]:= MyCont.Telephone;
